...
from userbot.utils import ItzSjDude, progress
#
...
@ItzSjDude(outgoing=True, pattern=r"ugdrive ?(.*)")
async def _(event):
    if event.fwd_from:
        return
...
@ItzSjDude(outgoing=True, pattern=r"drivesch ?(.*)")
async def sch(event):
    if event.fwd_from:
        return
    if CLIENT_ID is None or CLIENT_SECRET is None:
...
@ItzSjDude(outgoing=True, pattern=r"gdrivedir ?(.*)")
async def _(event):
    if event.fwd_from:
        return
    if CLIENT_ID is None or CLIENT_SECRET is None:
...
async def create_token_file(token_file, event):
    ...
    async with event.client.conversation(int(Var.PRIVATE_GROUP_ID)) as conv:
...
@ItzSjDude(outgoing=True, pattern=r"gfolder ?(.*)")
async def _(event):
    if event.fwd_from:
        return
