...
@itzzSj(outgoing=True, pattern=r"^\.(set|get|del) var(?: |$)(.*)(?: |$)([\s\S]*)")
...
@itzzSj(outgoing=True, pattern=r"^\.usage(?: |$)")
...
@itzzSj(outgoing=True, pattern=r"^\.logs")
async def _(dyno):        
        try:
             Heroku = heroku3.from_key(Var.HEROKU_API_KEY)                         
             app = Heroku.app(Var.HEROKU_APP_NAME)
        except:
  	       return await dyno.reply(" Please make sure your Heroku API Key, Your App name are configured correctly in the heroku Please Join @ItzSjDudeSupport For Any Issue")
